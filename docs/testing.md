...
`pnpm test:int` passes `--passWithNoTests`, so a change that selects no Postgres-backed suite still proves Compose start-up and migration from zero; `pnpm test` fails on an empty unit project on purpose.
