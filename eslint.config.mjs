...
// money: the pure packages that compute with money or quantities, the
// trading runtime that authorizes and executes it, the paper adapter that
// prices simulated fills and fees, and packages/db, which reads every
// base-unit column back as a `numeric` — a string, and so one `parseFloat`
// away from a balance that no longer reconciles.
const MONEY_GUARD_FILES = [
  "packages/adapter-paper/**/*.{ts,tsx}",
